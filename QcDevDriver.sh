...
if [ $# == 0 ]; then
   ...
   if [ $1 == "uninstall" ]; then
      ...
      if [ -d $DEST_QUD_PATH ]; then
         $QC_LN_RM_MK_DIR/rm -rf $DEST_QUD_PATH
         if [ ! -d $DEST_QUD_PATH  ]; then
            echo "Successfully removed $DEST_QUD_PATH"
         else
            echo "Failed to remove $DEST_QUD_PATH"
         fi
      else
         echo "$DEST_QUD_PATH does not exist, nothing to remove"
      fi
...
if [[ $QC_SECURE_BOOT_CHECK = "SecureBoot enabled" ]]; then
   ...
   if [ -f $DEST_SIGN_PATH/Signkey_pub.der ]; then
      if [[ $QC_PUBLIC_KEY_VERIFY = "$DEST_SIGN_PATH/Signkey_pub.der is already enrolled" ]]; then
         echo "=========================="
         echo "$DEST_SIGN_PATH/Signkey_pub.der is already enrolled"
         echo "=========================="
      else
         echo "=========================="
         echo "$DEST_SIGN_PATH/Signkey_pub.der is not enrolled. Please follow $DEST_SIGN_PATH/signReadme.txt"
         echo "=========================="
         exit 1
      fi
   else
...
# mute rmnet msg
echo "Qualcomm Gobi Net driver is installed at $DEST_INS_RMNET_PATH"
...
#Temporarily disable GobiNet driver. mute rmnet
